...
function crearTarea(descripcion, fechaLimite, prioridad) {
    ...
    tarea.mostrarEnHTML = () => {
        ...
        // wrap the main info in a span so flexbox can manage spacing
        let tareaHTML = `<span class="info"><input type="checkbox" class="chk-completada" data-id="${tarea.id}" data-action="mark" ${estado}> <span class="descripcion ${estiloCompelatada}">${tarea.descripcion}</span> | <span class="fecha ${estiloCompelatada}">${tarea.fechaLimite}</span>  | <span class="prioridad ${estiloCompelatada}">${tarea.prioridad}</span></span>` +
